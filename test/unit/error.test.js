...
const { ns, isHello, setDifference } = require('../../src/utils');
...
describe('MongoErrors', () => {
  ...
  let errorClassesFromEntryPoint = Object.fromEntries(
    Object.entries(require('../../src/index')).filter(
  ...
  let errorClassesFromErrorSrc = Object.fromEntries(
    Object.entries(require('../../src/error')).filter(
      ([key, value]) => key.endsWith('Error') && value.toString().startsWith('class')
    )
  );
  ...
  for (const errorName in errorClassesFromEntryPoint) {
    describe(errorName, () => {
      it(`name should be read-only`, () => {
        // Dynamically create error class with message
        let error = new errorClassesFromEntryPoint[errorName]('generated by test');
